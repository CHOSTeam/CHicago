...
// Last edited on December 25 of 2019, at 22:19 BRT
...
Boolean IpcCheckPort(PWChar name) {
	Boolean ret;
	Asm Volatile("int $0x3F" : "=a"(ret) : "0"(0x27), "b"((UIntPtr)name));
	return ret;
}

PIpcMessage IpcSendMessage(PWChar name, UInt32 msg, UIntPtr size, PUInt8 buf, IntPtr rport) {
	UIntPtr ret;
	Asm Volatile("int $0x3F" : "=a"(ret) : "0"(0x28), "b"((UIntPtr)name), "c"(msg), "d"(size), "S"((UIntPtr)buf), "D"(rport));
	return (PIpcMessage)ret;
}

Void IpcSendResponse(IntPtr handle, UInt32 msg, UIntPtr size, PUInt8 buf) {
	IntPtr discard;
	Asm Volatile("int $0x3F" : "=a"(discard) : "0"(0x29), "b"(handle), "c"(msg), "d"(size), "S"((UIntPtr)buf));
}

PIpcMessage IpcReceiveMessage(PWChar name) {
	UIntPtr ret;
	Asm Volatile("int $0x3F" : "=a"(ret) : "0"(0x2A), "b"((UIntPtr)name));
	return (PIpcMessage)ret;
}

PIpcMessage IpcReceiveResponse(IntPtr handle) {
	UIntPtr ret;
	Asm Volatile("int $0x3F" : "=a"(ret) : "0"(0x2B), "b"(handle));
	return (PIpcMessage)ret;
}
