...
static BOOLEAN IsPciRoot(EFI_DEVICE_PATH *DevicePath) {
	/* PciRoot(%d) is the same as ACPI(PNP0A03,%d). */
...
EFI_STATUS EfiMain(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
	...
	EFI_STATUS Status = InitializeGraphics();
	...
	if (EFI_ERROR((Status = gBS->HandleProtocol(ImageHandle, &gEfiLoadedImageProtocolGuid,
												(VOID**)&LoadedImage)))) {
		Print(L"EFI Loader Error: Couldn't get the boot device ESP handle: %r\n", Status);
		return Status;
	}
	...
	if (EFI_ERROR((Status = ReadFile(L"\\EFI\\CHOS\\BOOT.SIA", &Buffer, &Size)))) {
		return Status;
	}
	...
	if (EFI_ERROR((Status = SiaLoadKernel(Buffer, Size, &KernelEntry, &KernelStartPhys, &KernelStartVirt,
	...
	if (EFI_ERROR((Status = gBS->AllocatePages(AllocateAddress, EfiLoaderData,
											   (sizeof(BOOT_INFO) + 0xFFF) / 0x1000, &PhysLocation)))) {
		Print(L"EFI Loader Error: Couldn't allocate memory for the boot information struct: %r\n", Status);
		return Status;
	}
	...
	if (EFI_ERROR((Status = gBS->AllocatePages(AllocateAddress, EfiLoaderData,
		...
		Print(L"EFI Loader Error: Couldn't allocate memory for relocating the kernel SIA file: %r\n",
			  Status);
		return Status;
	}
...
#else
	Print(L"Relocated the kernel SIA file to the physical address 0x%llx (virtual address 0x%x)\n",
		  PhysLocation, NewBuffer);
#endif
#endif
	...
	BootInfo->EfiMainAddress = (UINTN)EfiMain;
	...
	UINTN DescSize = 0, MapCount = 0, MapKey = 0, MapSize = 0;
	UINT64 MaxAddress = 0, MaxSize = 0;
	UINT32 DescVer = 0;
	...
	if (EFI_ERROR((Status = gBS->AllocatePages(AllocateAddress, EfiLoaderData, MemMapPages, &PhysLocation)))) {
		Print(L"EFI Loader Error: Couldn't allocate memory for the memory map: %r\n", Status);
		return Status;
	}
	...
	if (EFI_ERROR((Status = CreateDirectory(&DirectoryPhys, &KernelEndPhys, &KernelEndVirt, Gop->Mode->FrameBufferBase,
											&FrameBufferVirt, Gop->Mode->FrameBufferSize, MaxAddress, &RegionsPhys,
											&RegionsVirt)))) {
		return Status;
	}
	...
	if (EFI_ERROR((Status = gBS->ExitBootServices(ImageHandle, MapKey)))) {
		Print(L"EFI Loader Error: Can't exit the EFI environment: %r\n", Status);
		return Status;
	}
	...
	asm volatile("cli");
	asm volatile("mov %0, %%ebx; mov %1, %%ecx" :: "r"(BootInfoVirt), "r"(KernelEntry) : "%ebx", "%ecx");
	asm volatile("mov %0, %%cr3" :: "a"(DirectoryPhys));
	asm volatile("mov %cr4, %eax; or $0x10, %eax; mov %eax, %cr4");
	asm volatile("mov %cr0, %eax; or $0x80010001, %eax; mov %eax, %cr0");
