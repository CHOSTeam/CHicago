...
IntN EfiToLower(IntN Character) { return Character >= 'A' && Character <= 'Z' ? Character + 0x20 : Character; }
Void *EfiZeroMemory(Void *Buffer, UIntN Size) { return EfiSetMemory8(Buffer, Size, 0); }
...
Boolean EfiCompareMemory(const Void *Buf1, const Void *Buf2, UIntN Size) {
    ...
    while (Size--) if (*buf1++ != *buf2++) return False;
    return True;
}
...
Void *EfiAllocateZeroPool(UIntN Size) {
    ...
    if (ret != Null) EfiZeroMemory(ret, Size);
    return ret;
}

Void *EfiAllocateCopyPool(const Void *Source, UIntN Size) {
    Void *ret = EfiAllocatePool(Size);
    if (ret != Null && Source != Null) EfiCopyMemory(ret, Source, Size);
    return ret;
}
...
Void EfiFreePool(Void *Pool) {
    ...
    if (Pool != Null) EfiBS->FreePool(Pool);
}
...
EfiMemoryDescriptor *EfiGetMemoryMap(UIntN *Count, UIntN *Key, UIntN *Size, UInt32 *Version) {
    /* This function is similar to EfiGetFileSize, as in we need to call the same function twice (one for getting the
    ...
    if (status != EFI_BUFFER_TOO_SMALL || (map = EfiAllocatePool(size)) == Null) return Null;
