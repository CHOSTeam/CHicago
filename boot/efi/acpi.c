 ...
 *
 * Created on March110 of 2021, at 17:26 BRT
 * Last edited on March 11 of 2021 at 18:57 BRT */
...
const Void *EfiGetAcpiTables(Boolean *Extended) {
     ...
     * to remember that if we can find an ACPI 1.0 RSDP, we should keep on searching (as we may find an ACPI 2.0 RSDP). */
    ...
    for (UIntN i = 0; i < EfiST->NumberOfTableEntries; i++) {
        if (EfiCompareMemory(&EfiCT[i].VendorGuid, &EfiAcpi20TableGuid, sizeof(EfiGuid))) {
            ...
            *Extended = True;
            ...
            rsdt = (const Void*)(UIntN)((AcpiRsdp*)EfiCT[i].VendorTable)->RsdtAddress;
            *Extended = False;
        }
    }
