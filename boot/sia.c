...
EFI_STATUS SiaLoadKernel(CHAR8 *Buffer, UINTN Size, UINTN *Entry, UINTN *StartPhys, UINTN *StartVirt,
	...
	if (EFI_ERROR((Status = SiaReadFile(Buffer, Size, File, 0, sizeof(ELF_HEADER), (CHAR8*)&ElfHeader)))) {
		Print(L"EFI Loader Error: Couldn't read the kernel ELF header: %r\n", Status);
		return Status;
	...
	if (ProgHeaders == NULL) {
		...
		return EFI_OUT_OF_RESOURCES;
											   ...
											   (CHAR8*)ProgHeaders)))) {
		Print(L"EFI Loader Error: Couldn't read the kernel file: %r\n", Status);
		return Status;
	}
	...
	for (UINTN i = 0; i < ElfHeader.ProgHeaderCount; i++) {
		...
		if (EFI_ERROR((Status = gBS->AllocatePages(AllocateAddress, EfiLoaderData, SectPages, &Location)))) {
			Print(L"EFI Loader Error: Couldn't allocate memory for reading the kernel file: %r\n", Status);
			return Status;
		}
	 ...
	 * header, and, if compiled with debugging enabled, print some info. */
