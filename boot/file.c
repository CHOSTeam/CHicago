 ...
 * Last edited on October 29 of 2020, at 20:57 BRT */
...
EFI_STATUS ReadFile(CHAR16 *FileName, CHAR8 **Buffer, UINTN *Size) {
	...
	if (EFI_ERROR((Status = FsRoot->Open(FsRoot, &File, FileName,
		...
		Print(L"EFI Loader Error: Failed to open '%s': %r\n", FileName, Status);
		return Status;
	}
	...
	*Size = Info->FileSize;
	...
	if (EFI_ERROR((Status = gBS->AllocatePages(AllocateAnyPages, EfiBootServicesData, BufferSize / 0x1000, &Location)))) {
		Print(L"EFI Loader Error: Couldn't allocate memory for reading '%s': %r\n", FileName, Status);
		File->Close(File);
		return Status;
	} else if (EFI_ERROR((Status = File->Read(File, Size, (VOID*)Location)))) {
		Print(L"EFI Loader Error: Couldn't read '%s': %r\n", FileName, Status);
