 ...
 * Last edited on July 04 of 2021 at 19:28 BRT */

#include <arch.h>
#include <arch/mmu.h>
#include <efi/lib.h>

static inline UInt32 MmuMakeEntry(EfiPhysicalAddress Physical, UInt8 Flags) {
    return Physical | MMU_PRESENT | (Flags & MAP_WRITE ? MMU_WRITE : 0);
}
...
static EfiStatus MmuMap(Void *Directory, Mapping **List, Mapping *Entry) {
    ...
    /* Similar to the arm64 code (and the amd64 code), but we only have two levels to worry about (the PD and the
     * PT). */
...
s:  level = (UIntN)Directory;
    ...
    while (size) {
        ...
        if ((((Entry->Virtual + start) >> 22) & 0x3FF) != (((Entry->Virtual + start - 0x1000) >> 22) & 0x3FF)) goto s;
    }
...
EfiStatus ArchInitCHicagoMmu(UInt16, Mapping **List, Void **Out) {
     ...
     * this part of the function arch-independent, as it already is arch-independent). */
