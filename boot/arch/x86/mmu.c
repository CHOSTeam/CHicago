 ...
 * Last edited on July 15 of 2021 at 12:18 BRT */

#include <arch.h>
#include <arch/mmu.h>
#include <efi/lib.h>

#define MMU_TYPE UIntN
...
#define MMU_DEST_LEVEL(Virtual, Physical, Size) (!((Virtual) & 0x3FFFFF) && !((Physical) & 0x3FFFFF) && \
                                                 Size >= 0x400000 ? 0 : 1)
#define MMU_MAKE_ENTRY(Physical, Flags, Level) (((Physical) & ~0xFFF) | MMU_PRESENT | (Level ? 0 : MMU_HUGE) | \
                                                ((Flags & MAP_WRITE) ? MMU_WRITE : 0))
#endif
