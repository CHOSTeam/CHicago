 ...
 * Last edited on November 29 of 2020, at 23:08 BRT */
...
EFI_STATUS CreateDirectory(UINTN *DirectoryPhys,
	...
	if (EFI_ERROR(Status)) {
		Print(L"EFI Loader Error: Couldn't allocate memory for creating the page directory: %r\n", Status);
		return Status;
	}
	...
	*KernelEndVirt += 0x1000;
	...
	for (; Current < *KernelEndPhys; Current += 0x400000) {
		Directory[(0xC0000000 + Current) >> 22] = (Current & ~0x3FFFFF) | 0x83;
	}
	...
	*FrameBufferVirt = 0xC0000000 + Current;
	...
	for (; FbCurrent < FrameBufferSize; FbCurrent += 0x400000) {
		Directory[(0xC0000000 + Current + FbCurrent) >> 22] = ((FrameBufferPhys + FbCurrent) & ~0x3FFFFF) | 0x83;
	}
	...
	*RegionsStartVirt = 0xC0000000 + Current + FbCurrent;
	...
	for (; Current < *KernelEndPhys + (RegionCount * 0x84) +
		...
		Directory[(0xC0000000 + Current + FbCurrent) >> 22] = (Current & ~0x3FFFFF) | 0x83;
	}
	...
	*KernelEndVirt = 0xC0000000 + Current + FbCurrent;
	...
	Directory[EfiMainAddress >> 22] = (EfiMainAddress & ~0x3FFFFF) | 0x83;
