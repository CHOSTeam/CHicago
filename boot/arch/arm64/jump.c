 ...
 * Last edited on February 04 of 2021 at 15:24 BRT */
...
__attribute__((noreturn)) Void ArchJumpIntoCHicago(CHBootInfo* BootInfo, UIntN Arg, UIntN Entry, UInt16 Features) {
    if (BootInfo == Null) {
        goto e;
    }

    UInt64 *dir = BootInfo->Directory, tcr = (Features & SIA_ARM64_48_BITS) ? 0xB5103510 : 0xB5193519,
           sp = (Arg + offsetof(CHBootInfo, KernelStack) + sizeof(BootInfo->KernelStack)) & -16, val, val2, el;
    ...
    if ((el >> 2) == 1) {
        /* We're already in EL1, so, let's initially disable paging (so that we don't crash while possibly changing
         * from 39-bits to 48-bits and vice-versa). */
    ...
    Asm Volatile("msr mair_el1, %0\n"
                 ...
                 "msr ttbr0_el1, %2\n"
                 "msr ttbr1_el1, %3\n"
                 "msr sctlr_el1, %4; isb\n" :: "r"(0xFFBB4400ull), "r"(tcr | ((val & 0x0F) << 32)), "r"(dir[0] + 1),
                                               "r"(dir[1] + 1), "r"(val2 | 0x01));
